...
def convert_to_arguments(x,y):

    output = torch.zeros_like(x)

    x1 = x.clone()
    x1[torch.abs(x1) < 1e-3] = 1e-3

    indicator_x_gzero = x1 > 0
    indicator_x_lzero = x1 < 0
    indicator_x_eqzero = x1 == 0

    indicator_y_gzero = y > 0
    indicator_y_lzero = y < 0

    output[indicator_x_gzero] = torch.atan(y/x1)[[indicator_x_gzero]]
    output[indicator_x_eqzero & indicator_y_gzero] = pi/2
    output[indicator_x_eqzero & indicator_y_lzero] = -pi / 2
    output[indicator_x_lzero & torch.logical_not(indicator_y_lzero)] = torch.atan(y/x1)[[indicator_x_lzero & torch.logical_not(indicator_y_lzero)]] + pi
    output[indicator_x_lzero & indicator_y_lzero] = torch.atan(y / x1)[indicator_x_lzero & indicator_y_lzero] - pi
...
class ConeProjection(nn.Module):
    ...
    def forward(self, source_embedding_axis, source_embedding_arg, r_embedding_axis, r_embedding_arg):
        ...
        re_heads = torch.cos(torch.cat((source_embedding_axis, source_embedding_arg), 0))
        im_heads = torch.sin(torch.cat((source_embedding_axis, source_embedding_arg), 0))

        re_relations = torch.cos(torch.cat((r_embedding_axis, r_embedding_arg), 0))
        im_relations = torch.sin(torch.cat((r_embedding_axis, r_embedding_arg), 0))

        ## projection
        prod_1 = re_heads * re_relations - im_heads * im_relations
        prod_2 = re_heads * im_relations + im_heads * re_relations

        projected_re_ax = prod_1.chunk(2)[0]
        projected_re_ap = prod_1.chunk(2)[1]

        projected_im_ax = prod_2.chunk(2)[0]
        projected_im_ap = prod_2.chunk(2)[1]

        ## transform complex numbers into arguments
        projected_embedding_ax = convert_to_arguments(projected_re_ax,projected_im_ax)  
        projected_embedding_ap = convert_to_arguments(projected_re_ap, projected_im_ap)

        return projected_embedding_ax, projected_embedding_ap
