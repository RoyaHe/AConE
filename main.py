...
from util import flatten_query, parse_time, set_global_seed, eval_tuple, relation_classifier
...
def parse_args(args=None):
    ...
    parser.add_argument('--save_checkpoint_steps', default=15000, type=int, help="save checkpoints every xx steps")
    ...
    parser.add_argument('--clip',default=0.5,type=float,help="max clip value")
...
def save_model(model, optimizer, save_variable_list, args, step):
    ...
    torch.save({
        **save_variable_list,
        ...
        os.path.join(args.save_path, 'checkpoint_'+str(step)+'steps')
    )
...
def main(args):
    ...
    tmp_str = "g-{}-mode-{}-b-{}-drop-{}-lr-{}".format(args.gamma, args.center_reg, args.batch_size, args.drop, args.learning_rate)
    ...
    test_symmetry_queries, test_inverse_queries, test_composition_queries, test_containment_queries, test_transtivity_queries, test_joint_queries, test_other_queries  = relation_classifier(test_queries,
                                                                                                inverse_relations_path = './data/NELL_inverse_relations.txt',
                                                                                                symmetry_relations_path = './data/NELL_symmetric_relations.txt',
                                                                                                composition_relations_path = './data/NELL_composition_relations.txt',
                                                                                                containment_relations_path = './data/NELL_containment_relations.txt',
                                                                                                transtivity_relations_path = './data/NELL_transtivity_relations.txt')
    ...
    if args.do_test:
        '''
        ## joint queries
        for query_structure in test_joint_queries:
            logging.info(query_name_dict[query_structure] + ": " + str(len(test_joint_queries[query_structure])))
        test_joint_queries = flatten_query(test_joint_queries)
        test_joint_dataloader = DataLoader(
            TestDataset(
                test_joint_queries,
                args.nentity,
                args.nrelation,
        ...
        ## composition queries
        for query_structure in test_composition_queries:
            logging.info(query_name_dict[query_structure] + ": " + str(len(test_composition_queries[query_structure])))
        test_composition_queries = flatten_query(test_composition_queries)
        test_composition_dataloader = DataLoader(
            TestDataset(
        ...
        ## other_queries
        for query_structure in test_other_queries:
            logging.info(query_name_dict[query_structure] + ": " + str(len(test_other_queries[query_structure])))
        test_other_queries = flatten_query(test_other_queries)
        test_other_dataloader = DataLoader(
            TestDataset(
                test_other_queries,
                args.nentity,
                args.nrelation,
        ...
        ## inverse_queries
        for query_structure in test_inverse_queries:
            logging.info(query_name_dict[query_structure] + ": " + str(len(test_inverse_queries[query_structure])))
        test_inverse_queries = flatten_query(test_inverse_queries)
        test_inverse_dataloader = DataLoader(
            TestDataset(
        ...
        ## symmetry_queries
        for query_structure in test_symmetry_queries:
            logging.info(query_name_dict[query_structure] + ": " + str(len(test_symmetry_queries[query_structure])))
        test_symmetry_queries = flatten_query(test_symmetry_queries)
        test_symmetry_dataloader = DataLoader(
            TestDataset(
        ...
        ## containment_queries
        for query_structure in test_containment_queries:
            logging.info(query_name_dict[query_structure] + ": " + str(len(test_containment_queries[query_structure])))
        test_containment_queries = flatten_query(test_containment_queries)
        test_containment_dataloader = DataLoader(
            TestDataset(
        ...
        ## transtivity_queries
        for query_structure in test_transtivity_queries:
            logging.info(query_name_dict[query_structure] + ": " + str(len(test_transtivity_queries[query_structure])))
        test_transtivity_queries = flatten_query(test_transtivity_queries)
        test_transtivity_dataloader = DataLoader(
            TestDataset(
    ...
    if args.checkpoint_path is not None:
        ...
        checkpoint = torch.load(os.path.join(args.checkpoint_path, 'checkpoint_300000steps'))
        ...
        logging.info('Ramdomly Initializing ConE Model...')
        init_step = 0
    ...
    if args.do_test:
        ...
        test_all_metrics = evaluate(model, test_easy_answers, test_hard_answers, args, test_dataloader, query_name_dict,
                                    'Test', step, writer)
        '''
        test_other_metrics = evaluate(model, test_easy_answers, test_hard_answers, args, test_other_dataloader, query_name_dict,
                                    'Test', step, writer)
        test_inverse_metrics = evaluate(model, test_easy_answers, test_hard_answers, args, test_inverse_dataloader, query_name_dict,
                                    'Test', step, writer)
        test_symmetry_metrics = evaluate(model, test_easy_answers, test_hard_answers, args, test_symmetry_dataloader, query_name_dict,
                                    'Test', step, writer)
        test_containment_metrics = evaluate(model, test_easy_answers, test_hard_answers, args, test_containment_dataloader, query_name_dict,
                                    'Test', step, writer)
        test_transtivity_metrics = evaluate(model, test_easy_answers, test_hard_answers, args, test_transtivity_dataloader, query_name_dict,
                                    'Test', step, writer)
        test_composition_metrics = evaluate(model, test_easy_answers, test_hard_answers, args, test_composition_dataloader, query_name_dict,
                                    'Test', step, writer)
        test_joint_metrics = evaluate(model, test_easy_answers, test_hard_answers, args, test_joint_dataloader, query_name_dict,
                                    'Test', step, writer)
        '''
